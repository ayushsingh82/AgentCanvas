...
export default function MyAgentsPage() {
  ...
  if (!walletAddress) {
    return (
      <div className="min-h-screen bg-white font-sans tracking-tight flex items-center justify-center px-4 pt-24">
  ...
  return (
    <div className="bg-white font-sans tracking-tight px-4 pt-24 pb-24">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left side - Create Agent */}
        <div className="order-2 lg:order-1">
          <div className="rounded-2xl p-8 border-2 border-black shadow-[8px_8px_0_0_rgba(0,0,0,1)] h-full flex flex-col justify-center" style={{ backgroundColor: '#FFD1B3' }}>
        ...
        <div className="order-1 lg:order-2">
          <div className="space-y-4">
            ...
            {agents.map((agent) => (
              <div
                key={agent.id}
                className="rounded-2xl p-6 border-2 border-black shadow-[6px_6px_0_0_rgba(0,0,0,1)] flex items-center justify-between" style={{ backgroundColor: '#FFD1B3' }}
              >
                <div className="flex-1">
                  <h3 className="text-lg font-black text-black mb-1">{agent.name}</h3>
                  {agent.description && (
                    <p className="text-sm text-black opacity-80 mb-2">{agent.description}</p>
                  )}
                ...
                {agent.agentChatURL ? (
                  <Link href={`/chat/${agent.id}`}>
                    <button className="ml-4 bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] px-6 py-2 rounded-lg text-sm font-bold text-black hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all duration-200">
                      Chat
                    </button>
                  </Link>
                ) : (
                  <button
                    disabled
                    className="ml-4 bg-gray-200 border-2 border-black px-6 py-2 rounded-lg text-sm font-bold text-black opacity-50 cursor-not-allowed"
                  >
                    Not Deployed
                  </button>
                )}
